...
### Примечание по повёрнутым страницам (Rotate=270)

- Начиная с версии 3.2 штамп корректно позиционируется на страницах,
  у которых в метаданных установлен поворот `/Rotate=270` при базовой
  портретной геометрии (видимая ширина меньше высоты). Никаких
  дополнительных действий со стороны пользователя не требуется.

## Устранение неполадок

### Проблема: Текст штампа не виден после сохранения
- **Описание**: После обработки PDF-файла текст штампа не отображается.
- **Статус**: **ИСПРАВЛЕНО** в версии 3.2.
- **Причина**: Проблема была вызвана ошибкой в устаревшей библиотеке `PyPDF2`, которая использовалась для работы с PDF.
- **Решение**: Программа была обновлена для использования современной библиотеки `pypdf`, что полностью устранило данную проблему.
